...
from cvnets.models.classification.config.mobilevitv3 import get_configuration
from cvnets.modules import InvertedResidual, MobileViTBlockv3
from utils import logger


@MODEL_REGISTRY.register(name="mobilevitv3", type="classification")
class MobileViTv3(BaseImageEncoder):
    """
    """

    def __init__(self, opts, *args, **kwargs) -> None:
        num_classes = getattr(opts, "model.classification.n_classes", 6)
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        group.add_argument(
            "--model.classification.mitv3.mode",
            type=str,
            default="small",
            choices=["xx_small", "x_small", "small"],
            help="MobileViTv3 mode. Defaults to small",
        )
        group.add_argument(
        ...
        group.add_argument(
            "--model.classification.mitv3.transformer-norm-layer",
            type=str,
            default="layer_norm",
            help="Normalization layer in transformer. Defaults to LayerNorm",
        )
        group.add_argument(
            "--model.classification.mitv3.no-fuse-local-global-features",
            action="store_true",
            help="Do not combine local and global features in MobileViT block",
        )
        group.add_argument(
            "--model.classification.mitv3.conv-kernel-size",
            type=int,
            default=3,
        ...
        group.add_argument(
            "--model.classification.mitv3.head-dim",
            type=int,
            default=None,
            help="Head dimension in transformer",
        )
        group.add_argument(
            "--model.classification.mitv3.number-heads",
            type=int,
            default=None,
    ...
    def _make_layer(
        ...
        **kwargs
        ...
        block_type = cfg.get("block_type", "mobilevitv3")
        ...
        **kwargs
        ...
        block.append(
            MobileViTBlockv3(
                ...
                dropout=getattr(opts, "model.classification.mitv3.dropout", 0.1),
                ffn_dropout=getattr(opts, "model.classification.mitv3.ffn_dropout", 0.0),
                attn_dropout=getattr(
                    opts, "model.classification.mitv3.attn_dropout", 0.1
                ),
                head_dim=head_dim,
                no_fusion=getattr(
                    opts,
                    "model.classification.mitv3.no_fuse_local_global_features",
                    False,
                ),
                conv_ksize=getattr(
                    opts, "model.classification.mitv3.conv_kernel_size", 3
                ),
            )
        )
